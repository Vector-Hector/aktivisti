...
interface Emits {
  (e: 'update:posters', posters: PosterDto[]): void
  (e: 'posterClick', poster: PosterDto): void
}
  ...
  const refreshSource = () => {
    ...
    ;(map.value.getSource(posterSourceId) as GeoJSONSource)?.setData(
      makeFeatureCollection(
        props.posters.filter((_, index) => index !== props.activePosterIndex),
        !active
      )
    )
    if (active) {
      ...
      ;(map.value.getSource(activePosterSourceId) as GeoJSONSource)?.setData(
        emptySource.data as FeatureCollection
      )
    }
  }
