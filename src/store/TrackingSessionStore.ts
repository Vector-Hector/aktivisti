...
class TrackingSessionStore extends Store<TrackingSessionState> {
  ...
  private toLocalStorageIdentifier(eventArea: number, address: string) {
    return escape(
      `${this.localStorageMetricsPrefix}.${eventArea}.${
        this.getState().trackingSessionId
      }.${address}`
    )
  }

  private loadTrackingSessionStoreFromLocalStorage() {
    this.state.trackingSessionId =
      localStorage.getItem(this.localStorageSessionIdKey) ?? null

    for (const key of Object.keys(localStorage)) {
      if (key.startsWith(this.localStorageMetricsPrefix)) {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const [_, eventAreaId, trackingSessionId, address] =
          unescape(key).split('.')
        ...
        if (
          !(eventAreaId in this.state.sessions[trackingSessionId].eventAreas)
        ) {
  ...
  private storeAddressInLocalStorage(
    ...
    localStorage.setItem(
      this.toLocalStorageIdentifier(eventArea, address),
      JSON.stringify(metricValueMap)
    )
  }
  ...
  public clear() {
    ...
    for (const key of Object.keys(localStorage)) {
      if (
        key === this.localStorageSessionIdKey ||
        key.startsWith(this.localStorageMetricsPrefix)
      ) {
  ...
  getMetricsForAddress(
    eventArea: number,
    address: AddressDetails
  ): MetricValueMap | undefined {
  ...
  private aggregateForMetricRecordAndEventArea(
    eventArea: number,
  ...
  async updateMetricsForAddress(
    ...
    if (oldMetrics) {
      ...
      metricsToUpdate.push(
        ...Object.entries(metricValueMap).map(([metricRecordId, value]) => {
          return {
            metricRecordId,
            value
          }
    ...
    for (const metric of metricsToUpdate) {
      ...
      const aggregatedMetricValue = this.aggregateForMetricRecordAndEventArea(
        eventArea,
        metricId
      )
      updatePromises.push(
        apiClient.eventMetricRecordSubmissions.create({
